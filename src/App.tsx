...
const Container = styled.div<{ theme: 'dark' | 'light' }>`
  background-color: ${(props) =>
    props.theme === 'dark' ? colordarkBlue900 : colorlightGray200};
...
const Background = styled.div`
  background-image: ${(props) =>
    props.theme === 'dark' ? `url(${darkBg})` : `url(${lightBg})`};
...
const FilterSection = styled.div`
  ...
  align-items: center;
  color: ${(props) =>
    props.theme === 'dark' ? colorlightGray400 : colorlightGray400};
  background-color: ${(props) =>
...
const StateControl = styled.div`
  ...
  button {
    ...
    &:hover {
      color: ${(props) =>
        props.theme === 'dark' ? colordarkGray100 : '#333333 '};
    }
            ...
            <DragDropContext onDragEnd={onDragEndHandler}>
              ...
              <FilterSection theme={theme}>
                ...
                <StateControl theme={theme}>
                  <button
                    className={stateStatus === 'all' ? 'active' : ''}
                    onClick={() => setStateStatus('all')}
                  >
                    All
                  </button>
                  <button
                    className={stateStatus === 'active' ? 'active' : ''}
                    onClick={() => setStateStatus('active')}
                  >
                    Active
                  </button>
                  <button
                    className={stateStatus === 'completed' ? 'active' : ''}
                    onClick={() => setStateStatus('completed')}
                  >
                    Completed
                  </button>
                </StateControl>
